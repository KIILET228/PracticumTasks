...
public:
    ...
    // Adds a new dog to the session, placed at a random point on a random road
    // of the session's map, with zero speed and the default (north) direction.
...
public:
    ...
    // Advances game time by `delta` on every active session, moving every dog
