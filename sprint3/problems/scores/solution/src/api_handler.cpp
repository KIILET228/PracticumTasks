...
StringResponse ApiHandler::HandleState(const StringRequest& req) const {
    ...
    try {
        ...
        for (const auto& [id, player_state] : state.players) {
            ...
            json::array bag;
            for (const auto& bag_item : player_state.bag) {
                json::object bag_item_obj;
                bag_item_obj["id"] = bag_item.id;
                bag_item_obj["type"] = bag_item.type;
                bag.push_back(std::move(bag_item_obj));
            }
            ...
            player_obj["score"] = player_state.score;
        ...
        body["lostObjects"] = std::move(lost_objects_json);
        return MakeJsonResponse(http::status::ok, body, version, keep_alive, include_body);
    } catch (const app::ApplicationError& err) {
        return MakeErrorResponse(http::status::unauthorized, err.GetCode(), err.what(), version, keep_alive,
                                 include_body);
...
StringResponse ApiHandler::HandleAction(const StringRequest& req) {
    const unsigned version = req.version();
    const bool keep_alive = req.keep_alive();
    ...
    if (!HasJsonContentType(req)) {
        return MakeErrorResponse(http::status::bad_request, "invalidArgument"sv, "Invalid content type"sv, version,
                                 keep_alive, true);
    }
    ...
    try {
        ...
        return MakeErrorResponse(http::status::bad_request, "invalidArgument"sv, "Failed to parse action"sv, version,
                                 keep_alive, true);
    }

    try {
        application_.SetPlayerAction(*token, move);
    } catch (const app::ApplicationError& err) {
        const auto status =
            (err.GetCode() == "unknownToken") ? http::status::unauthorized : http::status::bad_request;
        return MakeErrorResponse(status, err.GetCode(), err.what(), version, keep_alive, true);
    }
...
StringResponse ApiHandler::HandleTick(const StringRequest& req) {
    ...
    try {
        const auto value = json::parse(req.body());
        const auto& obj = value.as_object();
        const auto& delta_value = obj.at("timeDelta");
        if (delta_value.is_int64()) {
            time_delta_ms = delta_value.as_int64();
        } else if (delta_value.is_uint64()) {
            time_delta_ms = static_cast<std::int64_t>(delta_value.as_uint64());
        } else {
            throw std::runtime_error("timeDelta must be an integer");
        }
    } catch (const std::exception&) {
        return MakeErrorResponse(http::status::bad_request, "invalidArgument"sv, "Failed to parse tick request JSON"sv,
                                 version, keep_alive, true);
    }
